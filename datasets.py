...
class CLEVR_Dataset(Dataset):
    def __init__(self, data_path, transform, split='train', max_words=40, prompt="", scale=1.0, rag_store_path=None, k=5):
        ...
        self.default_feature_path = os.path.join(self.data_path,'res_feats_4', 'd_features')
        self.nsc_feature_path = os.path.join(self.data_path,'res_feats_4', 'nsc_features')
        self.sc_feature_path = os.path.join(self.data_path,'res_feats_4', 'sc_features')
        self.split = split
    ...
    def __getitem__(self, index):
        ...
        default_feature_path = os.path.join(self.default_feature_path, image_name + '.npy')
        sc_feature_path = os.path.join(self.sc_feature_path, image_name.replace('default', 'semantic') + '.npy')
        nsc_feature_path = os.path.join(self.nsc_feature_path, image_name.replace('default', 'nonsemantic') + '.npy')

        default_feature_data = np.load(default_feature_path)
        sc_feature_data = np.load(sc_feature_path)
        nsc_feature_data = np.load(nsc_feature_path)
        ...
        sc_reference_words = None
        nsc_reference_words = None
        
        out = {}
        out['sc_ref_words'] = sc_reference_words if sc_reference_words else "1"
        out['nsc_ref_words'] = nsc_reference_words if nsc_reference_words else "1"
        ...
        if self.split == 'train':
            ...
            if random.random() < 0.5:
                ...
                out['bef_feature'] = default_feature_data
                out['aft_feature'] = sc_feature_data
                out['nsc_feature'] = nsc_feature_data
            else:
                out['bef_image'] = no_image_data
                out['aft_image'] = aft_image_data
                out['nsc_image'] = bef_image_data
                out['bef_feature'] = nsc_feature_data
                out['aft_feature'] = sc_feature_data
                out['nsc_feature'] = default_feature_data
        else:
            out['bef_feature'] = default_feature_data
            out['aft_feature'] = sc_feature_data
            out['nsc_feature'] = nsc_feature_data
...
class SpotDataset(Dataset):
    def __init__(self, image_path, anno_path, racap_path, transform=None, split='train', prompt="", k=5, feature_path = None) -> None:
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        # print(os.path.join(self.feature_path, '%s.npy' % img_id + '.png'))
        bef_feature = np.load(os.path.join(self.feature_path, '%s.npy' % (img_id + '.png')))
        aft_feature = np.load(os.path.join(self.feature_path, '%s.npy' % (img_id + '_2' + '.png')))
        ...
        if self.split == 'train':
            ...
            output['bef_feature'] = bef_feature
            output['aft_feature'] = aft_feature
            output['caption'] = text
            output['lexicon'] = ref_words if ref_words else "1"
        else:
