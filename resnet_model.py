...
from ResnetViT import create_eva_vit_g
from FusionViT import create_fusion_eva_vit_g
...
class FINER_MLLM(Blip2Base):
    def __init__(
        ...
        num_query_token=32,
        llm_model="",
        prompt="the difference between the before image and after image is that",
        max_txt_len=128,
        ...
        self.fusion_visual_encoder, self.fusion_ln_vision = self.init_fusion_vision_encoder(
            vit_model, img_size, drop_path_rate, use_grad_checkpoint, vit_precision
        )
        ...
        #将fusion_vit_g的除了内部crossattn结构以外的所有结构都冻结  

        # lora.mark_only_lora_as_trainable(self.visual_encoder)
        for name, param in self.fusion_visual_encoder.named_parameters():
        ...
        for param in self.llm_model.backbone.parameters():
            param.requires_grad = False
        ...
        for name, param in self.llm_model.backbone.named_parameters():
            if param.requires_grad:
                print(name)

        # for name, module in self.fusion_visual_encoder.named_modules():
        #     if hasattr(module, 'weight') and module.weight.requires_grad:
        #         convert_weights_to_fp16(module)

        self.llm_model.config.pad_token_id = 0
        self.llm_model.config.bos_token_id = 1
        self.llm_model.config.eos_token_id = 2

        self.pre_llm_proj = nn.Linear(
            self.fusion_visual_encoder.embed_dim, 768
        )
    ...
    def forward(self, bef_feature, aft_feature, caption, ref_tokens=None, ref_texts=None):
        device = bef_feature.device
        with self.maybe_autocast():
            bef_image_embeds = self.ln_vision(self.visual_encoder(bef_feature))
            aft_image_embeds = self.ln_vision(self.visual_encoder(aft_feature))


            fusion_bef_image_embeds, fusion_aft_image_embeds = self.fusion_visual_encoder(bef_image_embeds, aft_image_embeds)
            fusion_bef_image_embeds = self.fusion_ln_vision(fusion_bef_image_embeds)
            fusion_aft_image_embeds = self.fusion_ln_vision(fusion_aft_image_embeds)
           
            inputs_llm = torch.cat([self.llm_proj(self.pre_llm_proj(fusion_bef_image_embeds)), 
                                    self.llm_proj(self.pre_llm_proj(fusion_aft_image_embeds))], dim=1)
            atts_llm = torch.ones(inputs_llm.size()[:-1], dtype=torch.long).to(device)

        self.llm_tokenizer.padding_side = "right"
        self.llm_tokenizer.truncation_side = 'left'
        text_input_tokens = self.llm_tokenizer(
            [self.prompt]*bef_feature.shape[0],
            return_tensors="pt",
            padding="longest",
            truncation=True,
        ...
        with self.maybe_autocast():
            outputs = self.llm_model(
                ...
                return_dict=True,
                labels=targets,
                decoder_ref_token_ids=None,
                decoder_ref_attention_mask=None,
            )
    ...
    def init_vision_encoder(self, model_name, img_size, drop_path_rate, use_grad_checkpoint, precision):
        visual_encoder = create_eva_vit_g(
            img_size, drop_path_rate, use_grad_checkpoint, precision, feature_dim=2048, feature_size=(7, 7)  
        )
        ln_vision = LayerNorm(visual_encoder.num_features)
        self.vit_name = model_name
        return visual_encoder, ln_vision
    
    def init_fusion_vision_encoder(self, model_name, img_size, drop_path_rate, use_grad_checkpoint, precision):
        visual_encoder = create_fusion_eva_vit_g(
            img_size, drop_path_rate, use_grad_checkpoint, precision
        )
        ln_vision = LayerNorm(visual_encoder.num_features)
        self.fusion_vit_name = model_name
        return visual_encoder, ln_vision
