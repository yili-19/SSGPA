...
import utils
import datasets
import resnet_model as diff_model
...
def get_dataset(data_name, split):
    ...
    if data_name == 'clevr':
        ...
        dataset = datasets.SpotDataset(image_path=os.path.join(args.spot_path, 'resized_images'),
                                            ...
                                            split=split,
                                            prompt=args.prompt,
                                            feature_path=os.path.join(args.spot_path, 'res_feats_4'),
        )
        return dataset
    elif data_name == "IER":
    ...
    with tqdm(total=len(dataloader), mininterval=60, disable=False) as t:
        ...
        for step, data in enumerate(dataloader):
            ...
            if args.dataset == "spot":
                bef_imgs = data['bef_img'].to(device)
                aft_imgs = data['aft_img'].to(device)
                bef_features = data['bef_feature'].to(device)
                aft_features = data['aft_feature'].to(device)
                captions = data['caption']
                ref_tokens = data['lexicon']

                optimizer.zero_grad()
                with autocast():
                    loss = model(bef_features, aft_features, captions, ref_tokens)
            
            elif args.dataset == "clevr":
                bef_imgs = data['bef_image'].to(device)
                aft_imgs = data['aft_image'].to(device)
                nsc_imgs = data['nsc_image'].to(device)
                bef_features = data['bef_feature'].to(device)
                aft_features = data['aft_feature'].to(device)
                nsc_features = data['nsc_feature'].to(device)
                ...
                optimizer.zero_grad()
                with autocast():
                    sc_loss = model(bef_features, aft_features, sc_captions, ref_tokens=None,)
                    nsc_loss = model(bef_features, nsc_features, nsc_captions, ref_tokens=None,)
    ...
    with torch.no_grad():
        for data in tqdm(loader, mininterval=60, disable=False):
            if args.dataset == "spot":
                ...
                img_ids = data['img_id']
                ref_tokens = data['lexicon']
                captions = model.generate(bef_features, aft_features, ref_tokens)
                ...
                sc_captions = model.generate(bef_features, aft_features, ref_tokens=sc_ref_tokens)
                nsc_captions = model.generate(bef_features, nsc_features, ref_tokens=nsc_ref_tokens) 
